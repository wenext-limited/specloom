...
| [x] | P9-T3 | Add CLI generate command | codex | P9-T2 | `crates/cli/src/main.rs`, `crates/cli/tests/commands.rs`, `crates/cli/tests/integration_smoke.rs` | `cargo test -p cli` | `<pending>` | Started 2026-03-04 15:53 CST; generate now runs full pipeline with deterministic text/json output and stable error exit handling |
